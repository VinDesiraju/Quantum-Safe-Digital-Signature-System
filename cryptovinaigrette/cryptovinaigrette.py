"""
Generate Unbalanced Oil and Vinegar parameters and keys
"""

# -------------------- IMPORTS and Definitions -------------------- #

import argparse, hashlib, dill
from datetime import datetime as dt
from field.GF256 import GF256, GF256Errors
...
class rainbowKeygen:
    ...
    def generate_publickey(self, save=''):
        ...
        if save != '':
            with open(save + 'Pub.pub', 'wb') as pubFile:
    ...
    def generate_privatekey(self, save=''):
        ...
        if save != '':
            with open(save + 'Priv.pem', 'wb') as privFile:
                    ...
                    for i in range(ol):
                        ...
                        for j in range(ol):
                            for k in range(vl):
                                ...
                                x[k]
                                equations[i][j] = GF256.add(equations[i][j],
                                                            GF256.multiply(privKey.F_layers[layer][i]['betas'][j][k],
                                                                           x[k]))
                        ...
                        for j in range(ol + vl):
                            if j < vl:
                                ...
                                x[j]
                                consts[i] = GF256.add(consts[i],
                                                      GF256.multiply(privKey.F_layers[layer][i]['gammas'][0][j], x[j]))
                            else:
                                equations[i][j - vl]
                                privKey.F_layers[layer][i]['gammas'][0][j]
...
if __name__ == '__main__':
    ...
    myKeyObject = rainbowKeygen(save='./')

    # signing the document using the private key
    start = dt.now()
    signature = rainbowKeygen.sign('Priv.pem', '../test/realCat.txt')
    end = dt.now()
    if args.v:
        print("Signed in", end - start, "seconds")

    # verifying the signed document using the public key
    start = dt.now()
    print("Signature verification :", rainbowKeygen.verify('Pub.pub', signature, '../test/realCat.txt'))
    end = dt.now()
    if args.v:
